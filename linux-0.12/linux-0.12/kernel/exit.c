 ...
 */
int badtask_ptr(struct task_struct *p)
{
	int 	i;
...
void audit_ptree()
{
	...
	for (i=1 ; i<NR_TASKS ; i++) {
		if (!task[i])
			continue;
		if (badtask_ptr(task[i]->p_pptr))
			printk("Warning, pid %d's parent link is bad\n",
				task[i]->pid);
		if (badtask_ptr(task[i]->p_cptr))
			printk("Warning, pid %d's child link is bad\n",
				task[i]->pid);
		if (badtask_ptr(task[i]->p_ysptr))
			printk("Warning, pid %d's ys link is bad\n",
				task[i]->pid);
		if (badtask_ptr(task[i]->p_osptr))
